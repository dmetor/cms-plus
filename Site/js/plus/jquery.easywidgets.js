...
(function($) {
    ...
    */
    ...
    $.fn.AddEasyWidgetFromRepository = function(html, placeId, nearItem, operation) {
        debugger;
        var canAdd = true;
        var widget = $(html);
        //var s = $.extend(true, $.fn.EasyWidgets.defaults, settings);
    ...
    */
    function PrepareSortablePlaces(sortableItems, settings) {
        ...
        $(s.selectors.places).sortable({
            ...
            stop: function(e, ui) {
                //$(ui.item).attr("widgettype") from module source or undefined //check dropped item has came from widget or widget repo ?
                //AddWidget('./WidgetProvider.htm', 'widget-place-1'); create new widget from sourcetype
                //$(ui.item).parent().attr("id") //dropped area
                if ($(ui.item).attr("widgettype") != undefined) {
                    var widgetType = $(ui.item).attr("widgettype");
                    var nearItem = null;
                    var operation = null; //after, or before
                    if ($(ui.item).prev().length > 0) {
                        nearItem = $(ui.item).prev();
                        operation = "after";
                    }
                    else if ($(ui.item).next().length > 0) {
                        nearItem = $(ui.item).next();
                        operation = "before";
                    }


                    var droppedHolder = $(ui.item).parent().attr("id");
                    if (widgetType == "HtmlContent1")
                        AddWidget('./WidgetProvider1.htm', droppedHolder, nearItem, operation);
                    else
                        AddWidget('./WidgetProvider2.htm', droppedHolder, nearItem, operation);
                    $(ui.item).fadeOut().remove();
                }
    ...
    function AddWidget(url, placeId, nearItem, operation) {
        $.get(url, function(html) {
            $.fn.AddEasyWidgetFromRepository(html, placeId, nearItem, operation);
        });
    }
